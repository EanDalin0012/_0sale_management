...
class MemberDropDownPage extends StatefulWidget {
  final Map vMember;
  const MemberDropDownPage({Key key, this.vMember}) : super(key: key);

  @override
  _MemberDropDownPageState createState() => _MemberDropDownPageState();
}

class _MemberDropDownPageState extends State<MemberDropDownPage> {
  ...
  Widget _buildListTile({
    ...
    var isCheck = false;
    if(widget.vMember != null && widget.vMember[MemberKey.id] == dataItem[MemberKey.id] ) {
      isCheck = true;
    }
    return ListTile(
  ...
  onItemChanged(String value) {
    var dataItems = vDataTmp.where((e) => e[MemberKey.name].toLowerCase().contains(value.toLowerCase())).toList();
    return dataItems;
  }
